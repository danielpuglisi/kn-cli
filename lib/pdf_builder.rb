...
module PdfBuilder
  def self.call # (m)
    kn = YAML.load_file("#{Rails.root}/db/kns/223.yml")
    ...
    logo = File.read("#{Rails.root}/app/assets/images/ilv-logo.svg")
    pdf = Prawn::Document.new(page_size: 'A4', left_margin: 65.4, right_margin: 43, top_margin: 28.3, bottom_margin: 15.5) do
      ...
      repeat :all do
        ...
        bounding_box([bounds.left, bounds.top - 15], width: 373.5) do
          fill_color "244061"
          font_size 24
          text "Kompetenznachweis Modul 223"

          move_down 14
          fill_color "000000"
          font_size 12
          text "Multi-User-Applikationen objektorientiert realisieren"
        end
      end
      ...
      bounding_box([bounds.left, bounds.top - 93], width: bounds.width) do
        ...
        title_rows = []
        competence_rows = []
        kn['competences'].each_with_index do |part, row_index|
          part.each_with_index do |competence, index|
            ...
            if index == 0
              ...
              competence_rows << [
                { content: "#{row_index + 1}", rowspan: rowspan, size: 12, font_style: :bold, align: :center, valign: :center, background_color: 'E0E0E0', padding: [0, 0, 6, 0], width: 24 },
            ...
            competence['items'].each do |item|
              item_title = item['title']
              # TODO: Add logic which checks title for ${variable} and then queries special_attributes for that variable value
              item_title.gsub!("%{absences_count}", 0.to_s)
              competence_rows << [{ content: item_title, width: bounds.width - 49.7 - 28.4, height: 19.7 }, { content: "0", size: 12, align: :center, width: 45.3, height: 19.7 }]
            end
          end
        end
        ...
        table(
          [
            ...
            [{ content: 'Erreichte Punkte', colspan: 2, align: :right, size: 11 }, { content: '0', align: :center, size: 12 }],
            [{ content: 'Note auf halbe oder ganze gerundet', font_style: :bold, colspan: 2, align: :right, size: 12 }, { content: '0', align: :center, size: 12 }],
          ],
        ) do
          cells.border_width = 2.2
          cells.border_color = 'FFFFFF'
